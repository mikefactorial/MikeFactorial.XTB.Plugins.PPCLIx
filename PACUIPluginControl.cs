    ...
    public partial class PACUIPluginControl : PluginControlBase, IGitHubPlugin
    {
        ...
        private string ExecutePacCommand(string command, bool streamOutputToLabel)
        {
            ...
            if (pProcess == null)
            {
                ...
                if (streamOutputToLabel)
                {
                    ...
                    pProcess.Start();
                    cmdStreamWriter = pProcess.StandardInput;
        ...
        private void SyncPacAuthWithConnection(ConnectionDetail connection)
        {
            ...
            WorkAsync(new WorkAsyncInfo
            {
                ...
                Work = (worker, args) =>
                {
                    authlist = ExecutePacCommand("auth list", false);
                        ...
                        if (auth.ToLower().Contains(connection.WebApplicationUrl.ToLower()))
                        {
                            ...
                            if (Int32.TryParse(auth.Substring(0, auth.IndexOf("]")).Replace("[", "").Replace("]", ""), out authIndex))
                            {
                                ...
                                if (this.InvokeRequired)
                                {
                                    this.Invoke(new MethodInvoker(delegate
                                    {
                                        MessageBox.Show($"An existing authentication profile was found for the current connection. The authentication profile has been selected to use for future commands.\n {authSelectResponse}");
                                    }));
                                }
                                else
                                {
                    ...
                    if (!connectionUpdated)
                    {
                        ...
                        UpdateCommandText();
                        MessageBox.Show("No existing authentication profile could be found for the current connection. Use pac auth create to create a new authentication prfoile. We've prefilled some of the values based on the current connection.");
                    }
                }
            });
        }
